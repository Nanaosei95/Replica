...
function F = calc_bnprices_merger13(p,param,startval)
...
%pi1 = @(p1,p2,p3,p4,p5) (p1-c)*s1(p1,p2,p3,p4,p5) + (p2-c)*s2(p1,p2,p3,p4,p5);
...
R3 = @(p2,p4,p5) fminunc(@(p13) -pi3(p13(1),p2,p13(2),p4,p5),[startval(1),startval(3)],fminopt); % Best response function
R4 = @(p1,p2,p3,p5) fminunc(@(p4) -pi4(p1,p2,p3,p4,p5),startval(4),fminopt); % Best response function
R5 = @(p1,p2,p3,p4) fminunc(@(p5) -pi5(p1,p2,p3,p4,p5),startval(5),fminopt); % Best response function

% Solver finds prices that make F=0
R3out = R3(p(2),p(4),p(5));

F(1) = R3out(1)-p(1);
F(2) = R2(p(1),p(3),p(4),p(5))-p(2);
F(3) = R3out(2)-p(3);
F(4) = R4(p(1),p(2),p(3),p(5))-p(4);
F(5) = R5(p(1),p(2),p(3),p(4))-p(5);
